...
import type { ObjectBlockProps } from "@/components/sections/ObjectBlock";
...
const fallbackHero: HeroHeaderProps = {
  name: "home-hero",
  carouselSpeed: 5000,
  ...
  linkBarItems: [
    { title: "Brand Identity", subtitle: "Positioning, identity & design systems" },
    { title: "Web & Product", subtitle: "Next.js, React & mobile applications" },
    { title: "Growth Strategy", subtitle: "Analytics, SEO & performance marketing" },
  ],
};

const fallbackContentBlock: ContentBlockProps = {
  name: "selected-work",
  title: "Selected work",
...
const fallbackObjectBlock: ObjectBlockProps = {
  name: "services",
  title: "What we do",
...
export default async function Home() {
  const [heroHeaders, contentBlocks, objectBlocks] = await Promise.all([
    getHeroHeaders(),
    getContentBlocks(),
    getObjectBlocks(),
  ]);

  const hero = heroHeaders[0] ?? fallbackHero;
  const contentBlock = contentBlocks[0] ?? fallbackContentBlock;
  const objectBlock = objectBlocks[0] ?? fallbackObjectBlock;
